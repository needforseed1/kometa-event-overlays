...
3. Update the local image tag in `compose.yml`.
4. Build without deploying:

   ```bash
   docker compose build --no-cache kometa-incremental
   ```
...
Stop the companion services, restore the prior image pin and patch, rebuild,
and start them again. A separate normal Kometa container is unaffected.
